...
int kensan(Number* f){  //カタラン定数をもとめるもう一つの計算  引数aに答えを返す
	...
	multiple(&k,&two,&a);
	add(&a,&one,&zet);  //2*k+1
	zeta(&zet,&c);  //c<=ζ(2*k+1)
	multiple(&c,&k,&c); //c<=c*k;
	power(&four,&a,&d); //d<=4^(2*k)

	divide(&c,&d,&e,&trash);  //ζ(2*k+1)*k/4^(2*k)=e
	...
	if(i>=20000){
		break;   //ぬけるところはもちろん要検討
	}
	}
	copyNumber(&ans,&f);  //引数aのところにコピーする
